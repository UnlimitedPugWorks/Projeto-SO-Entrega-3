...
*/
int readLineArguments(char **argVector, int vectorSize, char *buffer, int bufferSize, int fds)
{
  int numTokens = (fds == 0)?0:1;
  char *s = " \r\n\t";
  ...
  do{
    errno = 0;
    read(fds, buffer, bufferSize);
  }while(errno == EINTR);

  /* get the first token */
  if (fds != 0){
    message = strtok_r(buffer, "|", &buffer);
    argVector[0] = strtok_r(NULL, "|",&buffer);
  }
  else{
    message = buffer;
  }
  token = strtok_r(message, s, &message);
  /* walk through other tokens */
  while( numTokens < vectorSize-1 && token != NULL ) {
    argVector[numTokens] = token;
    numTokens ++;
    token = strtok_r(NULL, s, &message);
  }
